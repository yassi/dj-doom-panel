...
test_install: build
	python -m pip uninstall -y $(PACKAGE_NAME) || true
	python -m pip install -e .
	python -c "import dj_doom_panel; print('✅ Import success!')"
...
test_coverage:
	...
	@python -m pytest --cov=dj_doom_panel --cov-report=xml --cov-report=html --cov-report=term-missing tests/
...
test_coverage_docker:
	...
	@docker compose exec dev bash -c "cd /app && REDIS_HOST=redis python -m pytest --cov=dj_doom_panel --cov-report=xml --cov-report=html --cov-report=term-missing tests/"
...
docker_up:
	...
	@echo "   Dev container: dj-doom-panel-dev-1"
	@echo ""
